  ...
  get_null_distribution_perm <- function(data, idv = "id", dv = "rt", iv = "condition", params, f, perm_repetitions = 25, null_dist_samples = 10000, preprocessFs = c(), preprocessArgs = c()) {
    ...
    null_scores <- vapply(1:perm_repetitions, function(idx, data, idv, dv, iv, preprocessFs, preprocessArgs, params, f, pb) {
    ...
    get_null_sample <- function(iteration, pb) {
      ...
      sampled <- vapply(1:length(rndShuff), function(i) null_scores[i,rndShuff[i]], numeric(1))
    ...
    null_dist <- vapply(1:null_dist_samples, get_null_sample, numeric(1))
    return(null_dist)
  }
